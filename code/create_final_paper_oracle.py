...
def add_table_to_doc(doc, df, caption, font_size=7):
    """添加表格到文档"""
    # 添加表格标题
    p = doc.add_paragraph()
...
def main():
    ...
    # 加载 Oracle 实验修正后的文档
    original_doc_path = "/Volumes/agents/codex/paper2-LSTM量化金融/archive_20260710_1046_chatgpt_review/JFDS_Submission_Package_20260712/paper/main_paper_jfds_READY_v27_oracle.docx"
    ...
    # 定义所有表格（包含新增的Oracle实验表格）
    table_configs = [
        ('Table1_Source_Diversity.csv', 'Table 1: Source Policy Diversity'),
        ('Table2_Collapse_by_Source.csv', 'Table 2: Source Degeneracy and Target Outcome'),
        ...
        ('Data_Coverage_Table.csv', 'Table 6a: Data Coverage Summary'),
        ...
        ('Table14_Source_vs_Target.csv', 'Table 14: Source-Target Outcome Classification'),
        ('Table15_Robustness_Summary.csv', 'Table 15: Robustness Summary'),
        ('Table16_Context_Value.csv', 'Table 16: Incremental Value of Context'),
        ('Table17_Regime_Performance.csv', 'Table 17: Oracle Performance by Regime'),
        ('Table18_Context_Feature_Importance.csv', 'Table 18: Context Feature Importance'),
    ]
    ...
    # 保存文档
    output_file = "/Volumes/agents/codex/paper2-LSTM量化金融/archive_20260710_1046_chatgpt_review/JFDS_Submission_Package_20260712/paper/main_paper_jfds_FINAL_ORACLE.docx"
    new_doc.save(output_file)
    
    print(f"\n✅ 最终文档已保存: {output_file}")
    print(f"包含 {len(table_configs)} 个完整数据表格")
    print(f"包含 Oracle 和 Benchmark 实验")
